...
**当前推进**：**排队 14 · 全局功能页 UI 重设计**（用户 2026-05-26 拍板提前）。13.2 / 13.8 后放，先做纯前端无外部依赖的视觉重设计。
**新优先级**（2026-05-26 重排）：
...
### 第三阶段 · UI 重设计（**当前主线**，2026-05-26 提前）

#### 排队 14 — 全局功能页前端视觉重设计 · **当前主线 2026-05-26**
...
**无。当前主线 = 排队 14 UI 重设计**（2026-05-26 用户拍板提前）。

执行顺序：

1. **排队 14** 全局功能页 UI 重设计 ← **正在做**（纯前端无外部依赖，先推进）
2. **排队 13.2** TD-1 真 LLM provider ← 等用户拍上游 provider（DeepSeek / Qwen / Zhipu / Anthropic）+ Key/充值流程
3. **排队 13.8** TD-2 解析 pipeline + RAG 公告 ← 卡 13.2（必须先有真 LLM）
