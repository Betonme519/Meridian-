...
1. **E2 公共必修 40 学分组成的求和验证**：思政 17 + 英语 8 + 计算机 0/3/5 或师范 4 + 体育 4 + 国情 3 + 劳动 2 + 心理 2 = **36 / 39 / 41 / 40（师范）**。指南 §03 给出"40 学分左右"，跟实际求和**符合**（个别非师范生组合可能 36-41 浮动）。AI 顾问回答时直接用"约 40 学分"即可。
2. **师范生入 track 颗粒度**：用户拍板 `scope_level='college'`。需要 0005 seed SQL 决定 `track.college` 取值是 **"师范学院"（单一）** 还是 **按具体师范专业拆**（如 "心理与认知科学学院师范生" / "教育学部师范生"）。E4 段当前 metadata 写 `scope_level:"college"`，0005 录入时按用户实际场景决定 college 字段的颗粒度。
