...
1. **C3 强基段独立办法未在新 PDF 中找到 → 接受当前 4 条散见状态**（2026-05-16 用户拍板）。C3 段标题保留 ⚠️ 作为"source 不完整"标记（提醒后续若学校发布新版强基办法可补），但**不再是 pending decision**。0005 seed SQL 阶段 3 录入时 C3 段仅产 4 条 INSERT。等排队 13 AI prompt 落地若不够细则等学校发新版补。
