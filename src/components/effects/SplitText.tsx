...
interface SplitTextProps {
  ...
  tag?: HeadingTag;
  className?: string;
  /** Stagger between letters / words (ms). */
  delay?: number;
...
export default function SplitText({
    ...
    () => {
      ...
      const assignTargets = (self: GSAPSplitText) => {
        ...
        if (!targets && splitType.includes("words") && self.words?.length)
          targets = self.words as Element[];
        if (!targets && splitType.includes("lines") && self.lines?.length)
          targets = self.lines as Element[];
  ...
  // Use createElement so we can render any string-typed HTML tag without
  // wrestling the React 19 JSX namespace.
