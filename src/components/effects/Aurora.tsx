...
void main() {
  ...
  // 原式 auroraColor = intensity * rampColor：边缘 intensity→0 时颜色塌成黑色，而 alpha
  // 仍非 0 → 边缘出现一圈很深的黑边。改成给颜色加下限（0.6~1.0 之间随 intensity 变），
  // 边缘也显示真实色相、不再发黑；核心仍略亮。只动颜色、不动 auroraAlpha（形状不变）。
  vec3 auroraColor = rampColor * (0.6 + 0.4 * clamp(intensity, 0.0, 1.0));

  // 颗粒感：高频 hash 噪声（随帧轻微闪动）叠在颜色上做胶片颗粒。
  // 只影响有 alpha 的区域（最终乘 auroraAlpha），透明处不显噪点。
